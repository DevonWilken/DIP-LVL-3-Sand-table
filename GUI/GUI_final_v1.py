...
<Navigation>:
    BoxLayout:
        ...
        '''
image_list = []
playlist_15_min = ["Gcode\Spiral\Spiral.png", "Gcode\SpiralWeb\Spiral Web.png", "Gcode\Square\Square.png"]
list_of_lists = []
def play_gcode(file, list_of_lists):
    # Open g-code file
    f = open(file,'r')
    for line in f:
...
class Playing(Widget):
    ...
    def __init__(self, **kwargs):
        self.playlist_no = 0
        playing_image_source = self.playlist_15_min[self.playlist_no]
    ...
    def skip(self, event):
        ...
        if self.playlist_no >= 2:
            self.playlist_no = -1
        self.playing_image.source =  self.playlist_15_min[self.playlist_no]
...
class Playlist(BoxLayout):
    def __init__(self, **kwargs): 
        ...
        for item in playlist_15_min:
            ...
            self.card_image = FitImage(pos_hint= { "center_x": 0.5, "center_y": 0.5 }, source= item, size_hint= (None, None), size= (140, 140))
